...
#     two integers separated by a single space, which are the X and Y coordinate of the 
#     rook (1-indexed). You may assume no two rooks occupies the same sqaure.
#     1 <= N <= 1,000,000
#     1 <= X, Y <= N
#     0 <= K <= 100
...
#         Output will be an integer representing the number of sqaures that are not 
#     attacked.
#
# > Sample Input 1:
#     8
...
safe = N * N
safe -= N * (len(x_taken) + len(y_taken))
safe += len(x_taken) * len(y_taken)
